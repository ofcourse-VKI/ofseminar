...
void Foam::heavingAndLinearDeformingPointPatchVectorField::updateCoeffs()
{
    ...
    //const scalarField flag("flag", 0);	
    //scalarField flag = xCoord > xCoord/xCoord*0.06;
    scalarField flag1 = xCoord; // dummy init
    scalarField flag2 = xCoord; // dummy init

    forAll(xCoord, faceI)
    {
	flag1[faceI] = (xCoord[faceI] <=  (xoff_+0.01346)) ? 1.0 : 0.0;
        flag2[faceI] = (xCoord[faceI] >= (xoff_ +length_)) ? 1.0 : 0.0;
    }	
	...
	(1-flag1)*(1-flag2)*hp_*sin(2*M_PI*frequency_*t.value())*sqr(xCoord-xoff_)
        + flag2*(a0_*(xCoord)+a1_)*sin(2*M_PI*frequency_*t.value())
    );
